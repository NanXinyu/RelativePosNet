...
def inference(patches, cfg):
    '''
    input: [batch_size, num_joints, pacthes]
    ...
    num_h = num_patches // cfg.MODEL.PATCH_SIZE[1] # 64//8 = 8
    num_w = num_patches // cfg.MODEL.PATCH_SIZE[0] # 64//8 = 8
    print("num for heright:{}".format(num_h))
    print("num for width:{}".format(num_w))

    center_y = np.tile(0.5 * cfg.MODEL.IMG_SIZE[0], (batch_size, num_joints, num_patches)).astype(np.float32) # [32, 17, [128, 128]]
    center_x = np.tile(0.5 * cfg.MODEL.IMG_SIZE[1], (batch_size, num_joints, num_patches)).astype(np.float32)

    new_patch = [cfg.MODEL.IMG_SIZE[0]//num_h, cfg.MODEL.IMG_SIZE[1]//num_w] # [32, 17, [32, 32]]
    print("new patch size = {}".format(new_patch))
    
    idx = np.tile(np.arange(num_patches), (batch_size, num_joints, 1)).astype(np.float32)
    print("idx={}".format(idx))

    n_w = np.tile(num_w, (batch_size, num_joints, num_patches)).astype(np.float32) #8
    print("n_w={}".format(n_w))

    x = (idx % n_w) * new_patch[1] + np.tile(new_patch[1], (batch_size, num_joints, num_patches)) * 0.5
    print("x={}".format(x))
    y = (idx // n_w) * new_patch[0] + np.tile(new_patch[0], (batch_size, num_joints, num_patches)) * 0.5 
    print("y={}".format(y))
    ...
    pred_theta = (theta*patches).sum(axis = 2).reshape(batch_size, num_joints, 1)
    pred_dist = (dist*patches).sum(axis = 2).reshape(batch_size, num_joints, 1)
    print("pred theta={}".format(pred_theta*180/2/np.pi))
    print("pred dist={}".format(pred_dist))
    
    # pred_coordinate: [batch_size, num_joints, [x,y]]
    pred_x = center_x + pred_dist * np.cos(pred_theta)
    pred_y = center_y + pred_dist * np.sin(pred_theta)
